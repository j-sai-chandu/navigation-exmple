<div class="container w-full mx-auto flex flex-col items-center justify-center rounded-md shadow hover:shadow-lg">
    <div class="w-full bg-white px-4 py-3 border-b border-gray-100">
        <h3 class="text-lg leading-6 font-medium text-gray-800">
            @lang('Recent Subjects')
        </h3>
        <p class="max-w-2xl text-sm text-gray-500 mb-0">
            {{__('Recently published subjects')}}
        </p>
    </div>
    <ul class="w-full py-3">
        @foreach ($recentSubjects as $row)
        @php
        $details_url = route("frontend.subjects.show",[encode_id($row->id), $row->slug]);
        @endphp
